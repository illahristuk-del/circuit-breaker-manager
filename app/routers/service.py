...
from app.schemas.service import CreateService, ResponseService, ResponseCBStateResponse, ResponseHealthCheckLog
...
async def register_service(service_data: CreateService, db: DB_DEPENDS):
    try:
        new_service = await service_crud.create_service(db=db, service_data=service_data)
        return new_service
...
@router.post("/circuit_breaker/{service_id}/trip", response_model=ResponseCBStateResponse)
